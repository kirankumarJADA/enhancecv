...
router.get('/', requireAuth, async (req, res) => {
  ...
  const userResult = await db.query(
    'SELECT name, email, target_role, onboarded FROM users WHERE id = $1',
    [uid],
  );

  const user = userResult.rows[0] as
    | { name: string; email: string; target_role: string; onboarded: boolean }
    | undefined;
  ...
  const resumesResult = await db.query(
    `SELECT id, kind, title, ats_score, ${TS_TEXT('updated_at')} AS updated_at
     FROM resumes
     WHERE user_id = $1
  ...
  const resumes = resumesResult.rows as {
    id: string;
  ...
  const jobsResult = await db.query(
    `SELECT j.id, j.title, j.company, ${TS_TEXT('j.created_at')} AS created_at, ma.score AS match_score
  ...
  const latestAnalysisResult = await db.query(
    `SELECT ja.job_id, ja.analysis
     ...
     INNER JOIN (
       ...
       ON latest.job_id = ja.job_id
       AND latest.mx = ja.created_at
     WHERE ja.user_id = $1`,
    [uid],
  );

  const latestAnalysisRows = latestAnalysisResult.rows as {
    job_id: string;
    analysis: string;
  }[];
  ...
  const recentRunResult = await db.query(
    `SELECT id, job_id, version_id, before_ats, after_ats,
            before_match, after_match, truth_passed, ${TS_TEXT('created_at')} AS created_at
     FROM tailoring_runs
     WHERE user_id = $1
     ORDER BY created_at DESC
     LIMIT 1`,
  ...
  const recentRun = recentRunResult.rows[0] as
    | {
        id: string;
        job_id: string;
        version_id: string;
